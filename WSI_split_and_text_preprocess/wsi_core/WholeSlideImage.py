...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def process_contours_and_save_patches(self, 
                                          patch_output_root_dir: str, # 根目录，用于存放各个slide的patch文件夹
                                          patch_level: int = 0, 
                                          patch_size: int = 384, 
                                          step_size: int = 384, 
                                          contour_fn: str = 'four_pt', 
                                          use_padding: bool = True,
                                          white_thresh: int = 15, # from _getPatchGenerator defaults
                                          black_thresh: int = 50, # from _getPatchGenerator defaults
                                          white_black_filter: bool = True # whether to filter white/black patches
                                          ):
        """
        Processes tissue contours and saves valid patches directly as image files.
        Each slide's patches are saved in a subfolder named after the slide_id.
        """
        slide_patch_folder = os.path.join(patch_output_root_dir, str(self.name))
        os.makedirs(slide_patch_folder, exist_ok=True)
        
        print(f"Saving patches for WSI: {self.name} to folder: {slide_patch_folder}")
        total_patches_saved_for_slide = 0
        
        if self.contours_tissue is None:
            print(f"No tissue contours found for {self.name}. Skipping patch saving.")
            return 0

        for cont_idx, cont in enumerate(self.contours_tissue):
            if cont is None or len(cont) == 0:
                print(f"Contour {cont_idx} for {self.name} is empty. Skipping.")
                continue

            current_holes = self.holes_tissue[cont_idx] if self.holes_tissue and len(self.holes_tissue) > cont_idx else []

            start_x, start_y, w, h = cv2.boundingRect(cont)
            
            patch_downsample_at_level0 = (int(self.level_downsamples[patch_level][0]), int(self.level_downsamples[patch_level][1]))
            # ref_patch_size_at_level0 is the size of the patch in level 0 coordinates
            ref_patch_size_at_level0 = (patch_size * patch_downsample_at_level0[0], patch_size * patch_downsample_at_level0[1])
            
            img_w_level0, img_h_level0 = self.level_dim[0]

            if use_padding:
                stop_y_level0 = start_y + h
                stop_x_level0 = start_x + w
            else:
                stop_y_level0 = min(start_y + h, img_h_level0 - ref_patch_size_at_level0[1] + 1)
                stop_x_level0 = min(start_x + w, img_w_level0 - ref_patch_size_at_level0[0] + 1)
        
            if isinstance(contour_fn, str):
                if contour_fn == 'four_pt':
                    cont_check_fn = isInContourV3_Easy(contour=cont, patch_size=ref_patch_size_at_level0[0], center_shift=0.5)
                elif contour_fn == 'four_pt_hard':
                    cont_check_fn = isInContourV3_Hard(contour=cont, patch_size=ref_patch_size_at_level0[0], center_shift=0.5)
                elif contour_fn == 'center':
                    cont_check_fn = isInContourV2(contour=cont, patch_size=ref_patch_size_at_level0[0])
                elif contour_fn == 'basic':
                    cont_check_fn = isInContourV1(contour=cont)
                else:
                    print(f"Warning: Unknown contour_fn '{contour_fn}'. Defaulting to 'four_pt'.")
                    cont_check_fn = isInContourV3_Easy(contour=cont, patch_size=ref_patch_size_at_level0[0], center_shift=0.5)
            else:
                assert isinstance(contour_fn, Contour_Checking_fn)
                cont_check_fn = contour_fn
            
            step_size_x_level0 = step_size * patch_downsample_at_level0[0]
            step_size_y_level0 = step_size * patch_downsample_at_level0[1]

            patches_saved_for_contour = 0
            for y_coord_level0 in range(start_y, stop_y_level0, step_size_y_level0):
                for x_coord_level0 in range(start_x, stop_x_level0, step_size_x_level0):
                    if not WholeSlideImage.isInContours(cont_check_fn, (x_coord_level0, y_coord_level0), current_holes, ref_patch_size_at_level0[0]):
                        continue
                    
                    try:
                        # Read region at the specified patch_level with patch_size for that level
                        patch_pil = self.wsi.read_region(
                            (x_coord_level0, y_coord_level0), 
                            patch_level, 
                            (patch_size, patch_size) 
                        ).convert('RGB')

                        if white_black_filter:
                            if isBlackPatch(np.array(patch_pil), rgbThresh=black_thresh) or \
                               isWhitePatch(np.array(patch_pil), satThresh=white_thresh): 
                                continue
                        
                        # Filename uses level 0 coordinates
                        patch_filename = f"{x_coord_level0}_{y_coord_level0}.png"
                        patch_save_path = os.path.join(slide_patch_folder, patch_filename)
                        patch_pil.save(patch_save_path)
                        patches_saved_for_contour += 1
                    except Exception as e:
                        print(f"Error reading or saving patch at L0:({x_coord_level0},{y_coord_level0}) for slide {self.name}: {e}")
            
            if patches_saved_for_contour > 0:
                print(f"  Saved {patches_saved_for_contour} patches for contour {cont_idx}.")
            total_patches_saved_for_slide += patches_saved_for_contour

        print(f"Finished saving. Total {total_patches_saved_for_slide} patches saved for slide {self.name}.")
        return total_patches_saved_for_slide
